...
작업일자 : 2026-08-03
작업자 : 김동욱
목적 : 통신 고객 이탈 분류 모델을 학습하고 평가, 결과의 모델을 저장한다.
데이터 파일 : ./files/telecom_churn.csv
...
from sklearn.pipeline import Pipeline # 전처리+모델 하나로 묶어서 학습/예측 코드 재현성 보장
from sklearn.preprocessing import OneHotEncoder, StandardScaler

# 데이터 경로 설정
# __file__ : 현재 이 .py의 파일 자신의 경로
# .resolve() : 상대 경로를 절대 경로로 바꾸어준다. (어디서 실행하든지 안전)
# .parent : 파일이 들어있는 폴더
HERE = Path(__file__).resolve().parent
# DATA_PATH = 
# print(HERE / "files" / "telecom_churn.csv")
DATA_PATH = HERE / "files" / "telecom_churn.csv"

# print(HERE.parents[1]) # 인덱스 번호가 올라갈 수록 상위 폴더로 간다.

# 학습된 모델과 평가 지표를 이 파일이 있는 폴더(HERE) 바로 아래에 저장
...
def build_pipeline() -> Pipeline:
    ...
    numeric_pipe = Pipeline([
        ...
        # 표준화로 스케일링 : monthly_fee(수만원 단위), complaints(0~정도의 단위)처럼 단위차이가 클 때 사용
...
def train() -> dict:
    ...
    # predict_proba() : 각 클래스(0, 1)에 속할 확률을 만환 (0: 이탈안한다. 1: 이탈한다)
    #                   [:, 1] 모든 행, 1번 컬럼
    #                   이탈할 확률을 알고 싶다.
    probabilities = pipeline.predict_proba(X_test)[:, 1]

    # 확률이 0.5이상이면 이탈(1), 미만이면 비이탈(0)로 최종 분류
    predictions = (probabilities >= 0.5).astype(int) # int 형변환
    ...
    metrics = {
        ...
        # roc_auc : 0.5(랜덤 수준) ~ 1.0(완벽) 사이값, 임계값(0.5)에 상관없이 모델의 전반적인 분류 성능을 보여주는 지표
        # round(숫자, 4) : 숫자를 소수 넷째자리까지 반올림
        # float()를 붙여서 형변환을 하는 이유 : 파이썬의 기본 float이 아니라, numpy.float64 타입이라서
        "roc_auc": round(float(roc_auc_score(y_test, probabilities)), 4),
        # recall_churn : <재현율> 실제 이탈 고객 중 몇 %를 잡아냈냐? -> 이탈 예측에서는 이 값이 아주 중요!
        #                (놓치면 안되는 고객이므로)
    ...
    # 평가 지표를 사람이 읽을 수 있도록 JSON 파일로 저장
